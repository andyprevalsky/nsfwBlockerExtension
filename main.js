...
//     request.open("POST", "https://71cdeae3.ngrok.io/classify");
...
class VideoAnalyzer {
    ...
    handleSeek() {
        if (this.shadow.currentTime - this.video.currentTime < 1) {
            this.video.pause();
        }
        setTimeout(() => {
            this.playVideo()
        }, 1000);
        if(this.shadow.currentTime <  this.video.currentTime)
    ...
    timerCallback () {
        ...
        const shadowSecond = Math.floor(this.shadow.currentTime);
        const videoSecond = Math.floor(this.video.currentTime)

        if(!this.ratings[shadowSecond]) {
            this.ratings[shadowSecond] = .3 // get from model
        }

        if (this.ratings[videoSecond] > .6) showFullScreenCover();
